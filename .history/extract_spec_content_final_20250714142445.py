...
class SpecContentExtractor:
    ...
    def analyze_template_structure(self, paragraphs: List[str]) -> Dict[str, Dict]:
        ...
        print("\n--- Template Numbering Analysis ---")
        
        for text in paragraphs:
            # Extract numbering patterns from the text
            patterns = self.extract_numbering_patterns(text)
            
            if patterns:
                level_type, number = patterns[0]  # Take the first pattern found
                print(f"Found {level_type}: {number} in text: {text!r}")
                
                if level_type == "part":
                    current_part = f"Part_{number}"
                    current_subsection = None
                    
                elif level_type == "subsection":
                    current_subsection = f"Subsection_{number}"
                    ...
                    if key in template_structure:
                        ...
                        print(f"  Added item {number} to {key}")
                    ...
                    if key in template_structure:
                        ...
                        if template_structure[key]["expected_items"]:
                            ...
                            print(f"  Added list {number} to item {current_item} in {key}")
                            
                elif level_type == "sub_list" and current_part and current_subsection:
                    key = f"{current_part}:{current_subsection}"
                    if key in template_structure:
                        # Find the current item and list
                        if template_structure[key]["expected_items"]:
                            current_item = template_structure[key]["expected_items"][-1]
                            if current_item in template_structure[key]["expected_lists"]:
                                current_list = template_structure[key]["expected_lists"][current_item][-1]
                                print(f"  Found sub-list {number} under list {current_list} in item {current_item}")
        
        print("--- End Template Numbering Analysis ---\n")
