...
class SpecContentExtractor:
    ...
    def parse_paragraph_content(self, text: str) -> Tuple[Optional[str], Optional[str], str]:
        ...
        # Check for section title (specific known title or general pattern)
        if text == "Common Work Results for Electrical" or (
            len(text) > 10 and 
            text[0].isupper() and 
            not text.startswith("SECTION") and
            not text.startswith("GENERAL") and
            not text.startswith("PRODUCTS") and
            not text.startswith("EXECUTION")
        ):
