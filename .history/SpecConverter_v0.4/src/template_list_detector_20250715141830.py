...
class TemplateListDetector:
    ...
    def extract_level_data(self, level_element) -> Dict[str, Any]:
        """
    ...
    def extract_paragraph_properties(self, p_pr_elem) -> Dict[str, Any]:
    ...
    def find_bwa_list_levels(self, template_path: str, numbering_definitions: Dict[str, Any]) -> Dict[str, ListLevelInfo]:
        ...
        try:
            ...
            # Find BWA levels by numbering definitions
            for abstract_num_id, abstract_info in numbering_definitions.items():
                if abstract_info.get("bwa_label"):
                    for level_num, level_info in abstract_info.get("levels", {}).items():
                        level_key = f"abstract_{abstract_num_id}_level_{level_num}"
                        ...
                        list_level_info = ListLevelInfo(
                            ...
                            bwa_label=abstract_info.get("bwa_label"),
                            is_bwa_level=True
                        )
                        
                        bwa_list_levels[level_key] = list_level_info
