...
class SpecContentExtractorV3:
    ...
    def correct_level_type_based_on_numbering(self, level_type: str, numbering_id: Optional[str], 
        ...
        # If it was classified as content but has numbering, analyze what type it should be
        if level_type == "content" and numbering_id is not None:
            # Check if this looks like a subsection title based on characteristics:
            # 1. Short text (typically subsection titles are concise)
            # 2. All caps or title case (common for subsection titles)
            # 3. No obvious list-like content
            # 4. Not starting with common list patterns
            
            is_short_text = len(text_clean) < 100
            is_all_caps_or_title_case = (text_clean.isupper() or 
                                        text_clean.istitle() or 
                                        text_clean == text_clean.title())
            doesnt_start_with_list_patterns = not any(text_clean.upper().startswith(prefix) 
                                                     for prefix in ["SECTION", "PART", "GENERAL", "1.", "2.", "A.", "B."])
            no_list_indicators = not any(indicator in text_clean.lower() 
                                       for indicator in ["shall", "will", "must", "should", "note:", "example:"])
            
            # If it has numbering and looks like a subsection title, classify it as such
            if (is_short_text and is_all_caps_or_title_case and 
                doesnt_start_with_list_patterns and no_list_indicators):
                return "subsection_title"
            
            # Otherwise, if it has numbering, it's likely a list item
            # Remove the short text restriction - any numbered content should be classified as list
        ...
        if level_type == "list" and numbering_id is not None:
            # Check if this looks like a subsection title using the same logic
            is_short_text = len(text_clean) < 100
            is_all_caps_or_title_case = (text_clean.isupper() or 
                                        text_clean.istitle() or 
                                        text_clean == text_clean.title())
    ...
    def extract_content(self, docx_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for text, paragraph in paragraphs:
                ...
                # Normalize level_number: Use Word's numbering_level if available, otherwise use our logical mapping
                if numbering_level is not None:
                    # Use Word's numbering level directly
                    level_number = numbering_level
                # If no Word numbering but we have a logical level, keep it
