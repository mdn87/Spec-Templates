            ...
            try:
                ...
                if hasattr(normal_template, 'paragraph_format') and normal_template.paragraph_format:
                    if hasattr(normal_target, 'paragraph_format') and normal_target.paragraph_format:
                        ...
                        if pf_template.line_spacing:
                            pf_target.line_spacing = pf_template.line_spacing
                        if hasattr(pf_template, 'keep_with_next') and pf_template.keep_with_next is not None:
                            pf_target.keep_with_next = pf_template.keep_with_next
                        if hasattr(pf_template, 'keep_lines_together') and pf_template.keep_lines_together is not None:
                            pf_target.keep_lines_together = pf_template.keep_lines_together
                        if hasattr(pf_template, 'page_break_before') and pf_template.page_break_before is not None:
                            pf_target.page_break_before = pf_template.page_break_before
                        if hasattr(pf_template, 'widow_control') and pf_template.widow_control is not None:
                            pf_target.widow_control = pf_template.widow_control
...
def override_default_fonts(doc):
    """Override default fonts to prevent unwanted font inheritance"""
    try:
        print("Overriding default fonts to prevent unwanted inheritance...")
        
        # Override Normal style font to ensure no Courier inheritance
        if 'Normal' in doc.styles:
            normal_style = doc.styles['Normal']
            if hasattr(normal_style, 'font') and normal_style.font:
                # Set explicit font properties to prevent inheritance
                normal_style.font.name = 'Arial'
                normal_style.font.size = Pt(11)
                normal_style.font.bold = False
                normal_style.font.italic = False
                normal_style.font.underline = None
                normal_style.font.color.rgb = RGBColor(0, 0, 0)  # Black
                print("Set Normal style font to Arial 11pt")
        
        # Override Default Paragraph Font
        if 'Default Paragraph Font' in doc.styles:
            default_font_style = doc.styles['Default Paragraph Font']
            if hasattr(default_font_style, 'font') and default_font_style.font:
                default_font_style.font.name = 'Arial'
                default_font_style.font.size = Pt(11)
                print("Set Default Paragraph Font to Arial 11pt")
        
        # Also override any BWA styles that might have inherited unwanted fonts
        for style in doc.styles:
            if style.name and style.name.startswith('BWA-'):
                if hasattr(style, 'font') and style.font:
                    # Only override if font name is Courier or not set
                    if not style.font.name or 'Courier' in style.font.name:
                        style.font.name = 'Arial'
                        print(f"Fixed font for {style.name}: set to Arial")
        
    except Exception as e:
        print(f"Warning: Could not override default fonts: {e}")
...
# Override default fonts to prevent unwanted inheritance
override_default_fonts(doc)
