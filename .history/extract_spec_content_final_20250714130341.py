...
class SpecContentExtractor:
    ...
    def extract_items_and_lists(self, paragraphs: List[str], start_index: int, end_index: int) -> List[Dict]:
        ...
        for i in range(start_index, end_index + 1):
            ...
            elif level_type == "list" and current_item:
                current_item["lists"].append({
                    "list_number": number,
                    "text": content
                })
            elif level_type == "sub_list" and current_item and current_item["lists"]:
    ...
    def validate_numbering_sequence(self, level_type: str, number: str, content: str):
        ...
        if level_type == "part":
            ...
            if self.current_subsection_number:
                ...
                expected_item = chr(ord('A') + len(self.current_items))
                if number != expected_item:
                    self.add_error(
                        "Numbering Error",
                        f"Item numbering sequence broken",
                        content,
                        expected_item,
                        number
                    )
            ...
            if self.current_item_number:
                ...
                expected_list = str(len(self.current_lists) + 1)
                if number != expected_list:
                    self.add_error(
                        "Numbering Error",
                        f"List numbering sequence broken",
                        content,
                        expected_list,
                        number
                    )
