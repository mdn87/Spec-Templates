...
CONTENT_PATH  = 'SECTION 26 05 19.json'
FONT_NAME = 'Arial'
FONT_SIZE = 10
...
def generate_content_from_json(doc, json_data):
    ...
    # Add parts (use template's BWA multilevel list styles)
    if isinstance(json_data, dict) and 'parts' in json_data:
        for i, part in enumerate(json_data['parts']):
            ...
            # Add part title using BWA-PART style
            try:
                p3 = doc.add_paragraph(part.get('part_title', f'Part {i+1}'), style='BWA-PART')
                set_font_and_size(p3)
                print(f"DEBUG: Applied BWA-PART style to: {part.get('part_title', 'Unknown')}")
            except:
                # Fallback to basic paragraph
                p3 = doc.add_paragraph(part.get('part_title', f'Part {i+1}'))
                set_font_and_size(p3)
                print(f"DEBUG: Fallback to basic paragraph for: {part.get('part_title', 'Unknown')}")
            ...
            if 'subsections' in part:
                for j, sub in enumerate(part['subsections']):
                    ...
                    # Add subsection title using BWA-SUBSECTION style
                    try:
                        p4 = doc.add_paragraph(sub.get('title', f'Subsection {j+1}'), style='BWA-SUBSECTION')
                        set_font_and_size(p4)
                        print(f"DEBUG: Applied BWA-SUBSECTION style to: {sub.get('title', 'Unknown')}")
                    except:
                    ...
                    if 'items' in sub:
                        for k, item in enumerate(sub['items']):
                            ...
                            # Add main item text using BWA-Item style
                            try:
                                p5 = doc.add_paragraph(item.get('text', f'Item {k+1}'), style='BWA-Item')
                                set_font_and_size(p5)
                                print(f"DEBUG: Applied BWA-Item style to: {item.get('text', 'Unknown')[:30]}...")
                            except:
                                p5 = doc.add_paragraph(item.get('text', f'Item {k+1}'), style='List Paragraph')
                                set_font_and_size(p5)
                            
                            # Add lists using BWA-List style
                            if 'lists' in item:
                                for l, list_item in enumerate(item['lists']):
                                    print(f"DEBUG: Processing list item {l+1}: {list_item.get('text', 'Unknown')[:50]}...")
                                    try:
                                        p6 = doc.add_paragraph(list_item.get('text', f'List Item {l+1}'), style='BWA-List')
                                        set_font_and_size(p6)
                                        print(f"DEBUG: Applied BWA-List style to: {list_item.get('text', 'Unknown')[:30]}...")
                                    except:
                                        p6 = doc.add_paragraph(list_item.get('text', f'List Item {l+1}'), style='List Paragraph')
                                        set_font_and_size(p6)
                            
                            # Add sublists using BWA-SubList style
                            if 'sublists' in item:
                                for m, sublist_item in enumerate(item['sublists']):
                                    print(f"DEBUG: Processing sublist item {m+1}: {sublist_item.get('text', 'Unknown')[:50]}...")
                                    try:
                                        p7 = doc.add_paragraph(sublist_item.get('text', f'SubList Item {m+1}'), style='BWA-SubList')
                                        set_font_and_size(p7)
                                        print(f"DEBUG: Applied BWA-SubList style to: {sublist_item.get('text', 'Unknown')[:30]}...")
                                    except:
        ...
        p1 = doc.add_paragraph('PART 1', style='BWA-PART')
        set_font_and_size(p1)
        
        p2 = doc.add_paragraph('Test Subsection', style='BWA-SUBSECTION')
        set_font_and_size(p2)
