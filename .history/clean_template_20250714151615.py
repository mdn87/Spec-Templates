...
def extract_numbering_info(docx_path: str) -> Dict[str, Any]:
...
def clean_numbering_xml(numbering_info: Dict[str, Any]) -> str:
    ...
    try:
        ...
        for abstract_num_id, info in numbering_info["abstract_nums"].items():
            if info["has_bwa"]:
                ...
                new_abstract_num.set(f"{{{root.nsmap['w']}}}abstractNumId", abstract_num_id)
        ...
        for num_id, info in numbering_info["nums"].items():
            if not info["to_remove"]:
                ...
                new_num.set(f"{{{root.nsmap['w']}}}numId", num_id)
...
def clean_document_xml(docx_path: str, numbering_info: Dict[str, Any]) -> str:
