                        ...
                        for k, item in enumerate(sub['items']):
                            ...
                            if 'lists' in item:
                                for l, list_item in enumerate(item['lists']):
                                    ...
                                    p5 = doc.add_paragraph(list_item.get('text', f'List Item {l+1}'), style='List Paragraph')
                                    apply_basic_numbering(p5, 4)
                                    set_font_and_size(p5)
                            
                            # Add sublists (level 5)
                            if 'sublists' in item:
                                for m, sublist_item in enumerate(item['sublists']):
                                    print(f"DEBUG: Processing sublist item {m+1}: {sublist_item.get('text', 'Unknown')[:50]}...")
                                    p6 = doc.add_paragraph(sublist_item.get('text', f'SubList Item {m+1}'), style='List Paragraph')
                                    apply_basic_numbering(p6, 5)
