...
class SpecContentExtractor:
    ...
    def parse_paragraph_content(self, text: str) -> Tuple[Optional[str], Optional[str], str]:
        ...
        # Check for section header (must be at the beginning of content)
        ...
        # Section title should be the first substantial text after the section header
        # It should be a descriptive title, not a part or subsection
        if (not self.section_header_found and 
            len(text) > 5 and 
            text[0].isupper() and 
            not text.upper().startswith("SECTION") and
            not text.upper() in ["GENERAL", "PRODUCTS", "EXECUTION"] and
            not text.upper() in ["SCOPE", "EXISTING CONDITIONS", "CODES AND REGULATIONS", "DEFINITIONS",
