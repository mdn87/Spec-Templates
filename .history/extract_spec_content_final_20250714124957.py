...
class SpecContentExtractor:
    ...
    def process_enhanced_elements(self, numbered_elements: List[Dict], embedded_elements: List[Dict]) -> List[Dict]:
        ...
        for part_elem in part_elements:
            ...
            # Find subsections for this part
            part_subsections = [e for e in numbered_elements 
                              if e["level_type"] == "subsection" and 
                              e["number"].startswith(part_number.split('.')[0] + '.')]
            
            subsections = []
            for subsection_elem in part_subsections:
                subsection_number = subsection_elem["number"]
                subsection_title = subsection_elem["content"]
                
                # Find items for this subsection
                subsection_items = [e for e in numbered_elements 
                                  if e["level_type"] == "item" and 
                                  e["index"] > subsection_elem["index"]]
                
                # Find the next subsection to determine the end boundary
                next_subsection = None
                for s in part_subsections:
                    if s["index"] > subsection_elem["index"]:
                        if next_subsection is None or s["index"] < next_subsection["index"]:
                            next_subsection = s
                
                # Find lists for this subsection
                subsection_lists = [e for e in numbered_elements 
                                  if e["level_type"] == "list" and 
                                  e["index"] > subsection_elem["index"]]
                if next_subsection:
                    subsection_lists = [e for e in subsection_lists 
                                      if e["index"] < next_subsection["index"]]
                
                # Process items and their lists
                items = []
                for item_elem in subsection_items:
                    if next_subsection and item_elem["index"] >= next_subsection["index"]:
                        continue
                    
                    # Find lists for this item
                    item_lists = [e for e in subsection_lists 
                                if e["index"] > item_elem["index"]]
                    
                    # Find the next item to determine the end boundary
                    next_item = None
                    for i in subsection_items:
                        if i["index"] > item_elem["index"]:
                            if next_item is None or i["index"] < next_item["index"]:
                                next_item = i
                    
                    if next_item:
                        item_lists = [e for e in item_lists 
                                    if e["index"] < next_item["index"]]
