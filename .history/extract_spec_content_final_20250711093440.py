...
class SpecContentExtractor:
    ...
    def parse_paragraph_content(self, text: str) -> Tuple[Optional[str], Optional[str], str]:
        ...
        # Check if the text is exactly one of the part names or starts with a part name
        # This prevents matching part names that appear within longer text
        for part_name in part_names:
            # Check for exact match or patterns like "PART 1 - GENERAL", "1.0 GENERAL", etc.
            if (text.upper() == part_name or 
                text.upper().startswith(f"PART ") and part_name in text.upper() or
                re.match(rf'^\d+\.0?\s*[-]?\s*{part_name}$', text.upper())):
                
                # Try to extract existing numbering
                number_match = re.search(r'(\d+)\.0?\s*[-]?\s*', text.upper())
                if number_match:
                    part_number = f"{number_match.group(1)}.0"
                else:
                    # Auto-generate part number based on position
