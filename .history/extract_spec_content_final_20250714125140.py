...
class SpecContentExtractor:
    ...
    def extract_all_numbered_elements(self, paragraphs: List[str]) -> List[Dict]:
        ...
        for i, text in enumerate(paragraphs):
            ...
            if level_type in ["part", "part_title", "subsection", "subsection_title", "item", "list", "sub_list"]:
                ...
                print(f"Found {level_type}: {number} - {content[:50]}...")
    ...
    def extract_content_enhanced(self, docx_path: str) -> Dict[str, Any]:
        """Extract all content from the Word document using enhanced two-phase processing"""
        print(f"Extracting content from: {docx_path}")
        ...
        print("Phase 1: Extracting all numbered elements...")
        # Phase 1: Extract all numbered elements
        numbered_elements = self.extract_all_numbered_elements(self.all_paragraphs)
        print(f"Found {len(numbered_elements)} numbered elements")
        
        print("Phase 1: Reconstructing broken lists...")
        # Phase 1: Reconstruct broken lists
        reconstructed_elements = self.reconstruct_broken_lists(numbered_elements)
        print(f"Reconstructed {len(reconstructed_elements) - len(numbered_elements)} elements")
        
        print("Phase 2: Scanning for embedded numbering...")
        # Phase 2: Scan for embedded numbering
        embedded_elements = self.scan_content_for_embedded_numbering(self.all_paragraphs, reconstructed_elements)
        print(f"Found {len(embedded_elements)} embedded elements")
        
        # Process the enhanced elements into the final structure
        self.extracted_data["parts"] = self.process_enhanced_elements(reconstructed_elements, embedded_elements)
        
        return self.extracted_data
    
    def process_enhanced_elements(self, numbered_elements: List[Dict], embedded_elements: List[Dict]) -> List[Dict]:
        """Process enhanced elements into the final hierarchical structure"""
        parts = []
        
        # Sort elements by index to maintain document order
        numbered_elements.sort(key=lambda x: x["index"])
        
        # Group elements by part (handle both part and part_title types)
        part_elements = [e for e in numbered_elements if e["level_type"] in ["part", "part_title"]]
        print(f"Processing {len(part_elements)} parts: {[f'{p['number']} - {p['content']}' for p in part_elements]}")
        
        for i, part_elem in enumerate(part_elements):
            part_number = part_elem["number"]
            part_title = part_elem["content"]
            print(f"Processing part {part_number}: {part_title}")
            
            # Determine the end boundary for this part
            part_end_index = len(numbered_elements) - 1
            if i + 1 < len(part_elements):
                part_end_index = part_elements[i + 1]["index"] - 1
            
            # Find subsections for this part (within the part boundaries, handle both subsection and subsection_title types)
            part_subsections = []
            for elem in numbered_elements:
                if (elem["level_type"] in ["subsection", "subsection_title"] and 
                    elem["number"] and elem["number"].startswith(part_number.split('.')[0] + '.') and
                    elem["index"] >= part_elem["index"] and
                    elem["index"] <= part_end_index):
                    part_subsections.append(elem)
            
            print(f"  Found {len(part_subsections)} subsections for part {part_number}")
            
            # Sort subsections by their number
            part_subsections.sort(key=lambda x: x["number"])
            
            subsections = []
            for j, subsection_elem in enumerate(part_subsections):
                subsection_number = subsection_elem["number"]
                subsection_title = subsection_elem["content"]
                print(f"    Processing subsection {subsection_number}: {subsection_title}")
                
                # Determine the end boundary for this subsection
                subsection_end_index = part_end_index
                if j + 1 < len(part_subsections):
                    subsection_end_index = part_subsections[j + 1]["index"] - 1
                
                # Find items for this subsection (within the subsection boundaries)
                subsection_items = []
                for elem in numbered_elements:
                    if (elem["level_type"] == "item" and 
                        elem["index"] > subsection_elem["index"] and
                        elem["index"] <= subsection_end_index):
                        subsection_items.append(elem)
                
                print(f"      Found {len(subsection_items)} items for subsection {subsection_number}")
                
                # Sort items by their letter
                subsection_items.sort(key=lambda x: x["number"])
                
                # Process items and their lists
                items = []
                for k, item_elem in enumerate(subsection_items):
                    # Determine the end boundary for this item
                    item_end_index = subsection_end_index
                    if k + 1 < len(subsection_items):
                        item_end_index = subsection_items[k + 1]["index"] - 1
                    
                    # Find lists for this item (within the item boundaries)
                    item_lists = []
                    for elem in numbered_elements:
                        if (elem["level_type"] == "list" and 
                            elem["index"] > item_elem["index"] and
                            elem["index"] <= item_end_index):
                            item_lists.append(elem)
                    
                    # Sort lists by their number
                    item_lists.sort(key=lambda x: int(x["number"]) if x["number"].isdigit() else 0)
                    
                    # Convert lists to the expected format
                    lists = []
                    for list_elem in item_lists:
                        lists.append({
                            "list_number": list_elem["number"],
                            "text": list_elem["content"]
                        })
                    
                    items.append({
                        "item_number": item_elem["number"],
                        "text": item_elem["content"],
                        "lists": lists
                    })
...
def main():
    ...
    print("Starting enhanced extraction with two-phase list reconstruction...")
    data = extractor.extract_content_enhanced(docx_path)
    ...
    print(f"Total errors: {len(extractor.errors)}")
