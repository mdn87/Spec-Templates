...
class SpecContentExtractor:
    ...
    def recover_broken_structure(self, analysis: List[Dict]) -> List[Dict]:
        ...
        # First, mark all existing items as processed to avoid duplicates
        for item in analysis:
            if item['level_type'] in ['item', 'subsection', 'subsection_title']:
        ...
        for i, item in enumerate(analysis):
            ...
            if item['level_type'] == 'item' and item['number']:
                ...
                if next_item:
                    ...
                    if line_index not in processed_indices:
    ...
    def extract_content(self, docx_path: str) -> Dict[str, Any]:
        """Extract all content from the Word document using two-pass approach with recovery"""
        ...
        # Second pass: Analyze structure and attempt recovery
        print("Analyzing document structure...")
        analysis = self.analyze_paragraph_structure(self.all_paragraphs)
        
        # Third pass: Attempt to recover broken structures
        print("Attempting to recover broken structures...")
        analysis = self.recover_broken_structure(analysis)
        
        # Fourth pass: Process the enhanced analysis
        ...
        for item in analysis:
            ...
            # Log recovery if this was a recovered item
            if item.get('recovered', False):
                print(f"RECOVERED (line {self.line_count}): {level_type} - {text[:50]}...")
