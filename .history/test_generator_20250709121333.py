...
def generate_content_from_json(doc, json_data):
    ...
    # Add parts (start multilevel list numbering here)
    if isinstance(json_data, dict) and 'parts' in json_data:
        for i, part in enumerate(json_data['parts']):
            ...
            # Add part title (level 0 - starts at 1.0)
            p3 = doc.add_paragraph(part.get('part_title', f'Part {i+1}'), style='Heading 2')
            apply_basic_numbering(p3, 0)
            ...
            if 'subsections' in part:
                for j, sub in enumerate(part['subsections']):
                    ...
                    # Add subsection title (level 1 - continues numbering)
                    p4 = doc.add_paragraph(sub.get('title', f'Subsection {j+1}'), style='Heading 3')
                    apply_basic_numbering(p4, 1)
                        ...
                        for k, item in enumerate(sub['items']):
                            ...
                            if 'lists' in item:
                                for l, list_item in enumerate(item['lists']):
                                    ...
                                    apply_basic_numbering(p6, 3)
                                    set_font_and_size(p6)
                            
                            # Add sublists (level 4)
                            if 'sublists' in item:
