        ...
        with zipfile.ZipFile(docx_path) as zf:
            ...
            if "word/numbering.xml" in zf.namelist():
                ...
                for abstract_num in root.findall(".//w:abstractNum", ns):
                    ...
                    # Check for list name - look at the first level pattern
                    first_level = abstract_num.find("w:lvl", ns)
                    list_name = ""
                    if first_level is not None:
                        lvl_text_elem = first_level.find("w:lvlText", ns)
                        if lvl_text_elem is not None:
                            list_name = lvl_text_elem.get(f"{{{ns['w']}}}val", "")
                    
                    # For now, let's keep all numbering definitions since none have "BWA" in them
                    # You can modify this logic based on your specific requirements
                    has_bwa = "BWA" in list_name.upper() or "PART" in list_name.upper() or "%1.0" in list_name
...
def create_cleaned_docx(original_path: str, numbering_info: Dict[str, Any]) -> Optional[str]:
    ...
    try:
        ...
        # Create a new zip file with cleaned content
        with zipfile.ZipFile(original_path, 'r') as original_zip:
            with zipfile.ZipFile(output_path, 'w') as new_zip:
                # Copy all files except numbering.xml and document.xml
                for item in original_zip.infolist():
                    if item.filename not in ["word/numbering.xml", "word/document.xml"]:
                        new_zip.writestr(item, original_zip.read(item.filename))
                
                # Add cleaned numbering.xml
                cleaned_numbering = clean_numbering_xml(numbering_info)
                if cleaned_numbering:
                    new_zip.writestr("word/numbering.xml", cleaned_numbering)
                    print(f"Added cleaned numbering.xml")
                
                # Add cleaned document.xml
                cleaned_document = clean_document_xml(original_path, numbering_info)
                if cleaned_document:
                    new_zip.writestr("word/document.xml", cleaned_document)
                    print(f"Updated document.xml")
                else:
                    # If cleaning failed, copy original document.xml
                    new_zip.writestr("word/document.xml", original_zip.read("word/document.xml"))
                    print(f"Copied original document.xml")
